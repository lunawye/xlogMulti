...
SOCKET ProxyTest::__Connect(const marsMulti::comm::ProxyInfo& _proxy_info, const std::string& _host, const std::vector<std::string>& _hardcode_ips) {
    NetSource::DnsUtil dns_util_;
    
    std::string proxy_ip;
    if (marsMulti::comm::kProxyNone != _proxy_info.type) {
    ...
    if (marsMulti::comm::kProxyHttp == _proxy_info.type) {
    ...
    if (marsMulti::comm::kProxyNone != _proxy_info.type && marsMulti::comm::kProxyHttp != _proxy_info.type) {
...
int ProxyTest::__ReadWrite(SOCKET _sock, const marsMulti::comm::ProxyInfo& _proxy_info, const std::string& _host) {
    
    std::map<std::string, std::string> headers;
    headers[http::HeaderFields::KStringHost] = _host;
    
    if (_proxy_info.IsValid() && marsMulti::comm::kProxyHttp == _proxy_info.type && !_proxy_info.username.empty() && !_proxy_info.password.empty()) {
    ...
    if (marsMulti::comm::kProxyHttp == _proxy_info.type && !_proxy_info.username.empty() && !_proxy_info.password.empty()) {
        std::string account_info = _proxy_info.username + ":" + _proxy_info.password;
        size_t dstlen = modp_b64_encode_len(account_info.length());
