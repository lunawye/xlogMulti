...
void TaskIntercept::AddInterceptTask(const std::string& _name, const std::string& _data) {
    qm_xinfo2(TSF"save task name %_", _name);
    if (_name.empty()) {
        return;
    }
...
bool TaskIntercept::GetInterceptTaskInfo(const std::string& _name, std::string& _last_data) {
    ...
    qm_xinfo2(TSF"find intercepted task: %_, time: %_, current: %_, data size %_", _name, info->second.intercept_time, 
                current_time, info->second.data.size());
    qm_xdebug2(TSF"intercepted last delte: %_", (current_time - info->second.intercept_time));
