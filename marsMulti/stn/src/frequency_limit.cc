...
#include "marsMulti/comm/adler32.h"
...
bool FrequencyLimit::Check(const marsMulti::stn::Task& _task, const void* _buffer, int _len, unsigned int& _span) {
    qm_xverbose_function();

    if (!_task.limit_frequency) return true;

    unsigned long time_cur = ::gettickcount();
    qm_xassert2(time_cur >= itime_record_clear_);
    unsigned long interval = time_cur - itime_record_clear_;

    if (RUN_CLEAR_RECORDS_INTERVAL_MINUTE <= interval) {
        qm_xdebug2(TSF"__ClearRecord interval=%0, timeCur=%1, itimeRecordClear=%2", interval, time_cur, itime_record_clear_);
    ...
    if (0 <= find_index) {
        ...
        if (!__CheckRecord(find_index)) {
            qm_xerror2(TSF"Anti-Avalanche had Catch Task, Task Info: ptr=%0, cmdid=%1, need_authed=%2, cgi:%3, channel_select=%4, limit_flow=%5",
                    &_task, _task.cmdid, _task.need_authed, _task.cgi, _task.channel_select, _task.limit_flow);
            qm_xerror2(TSF"apBuffer Len=%0, Hash=%1, Count=%2, timeLastUpdate=%3",
        ...
        qm_xdebug2(TSF"InsertRecord Task Info: ptr=%0, cmdid=%1, need_authed=%2, cgi:%3, channel_select=%4, limit_flow=%5",
                &_task, _task.cmdid, _task.need_authed, _task.cgi, _task.channel_select, _task.limit_flow);
...
void FrequencyLimit::__ClearRecord() {
    qm_xdebug2(TSF"iarrRecord size=%0", iarr_record_.size());
    ...
    while (first != iarr_record_.end()) {
        qm_xassert2(time_cur >= first->time_last_update_);
        ...
        if (interval <= NOT_CLEAR_INTERCEPT_INTERVAL_MINUTE && NOT_CLEAR_INTERCEPT_COUNT <= first->count_) {
            ...
            qm_xwarn2(TSF"timeCur:%_,  first->timeLastUpdate:%_, interval:%_, Hash:%_, oldcount:%_, Count:%_", time_cur, first->time_last_update_, interval, first->hash_, oldcount, first->count_);
            ++first;
        } else {
...
void FrequencyLimit::__UpdateRecord(int _index) {
    qm_xassert2(0 <= _index && (unsigned int)_index < iarr_record_.size());
