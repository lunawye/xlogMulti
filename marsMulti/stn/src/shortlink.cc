...
#include "marsMulti/comm/crypt/ibase64.h"
#include "marsMulti/baseevent/baseprjevent.h"
#include "marsMulti/comm/move_wrapper.h"

#if defined(__ANDROID__) || defined(__APPLE__)
#include "marsMulti/comm/socket/getsocktcpinfo.h"
#endif
#include "marsMulti/stn/proto/shortlink_packer.h"
...
class ShortLinkConnectObserver : public MComplexConnect {
  public:
    ...
    virtual void OnConnected(unsigned int _index, const socket_address& _addr, SOCKET _socket, int _error, int _rtt) {
        ...
        if (0 != _error) {
//            qm_xassert2(shortlink_.func_network_report);
...
ShortLink::ShortLink(MessageQueue::MessageQueue_t _messagequeueid, NetSource& _netsource, const Task& _task, bool _use_proxy, std::unique_ptr<SocketOperator> _operator)
    ...
    {
    qm_xinfo2(TSF"%_, handler:(%_,%_), long polling: %_ ",this, asyncreg_.Get().queue, asyncreg_.Get().seq, _task.long_polling);
}

ShortLink::~ShortLink() {
    if (task_.priority >= 0) {
        qm_xdebug_function(TSF"taskid:%_, cgi:%_, @%_", task_.taskid, task_.cgi, this);
    }
...
void ShortLink::SendRequest(AutoBuffer& _buf_req, AutoBuffer& _buffer_extend) {
    ...
    qm_xdebug2(XTHIS)(TSF"bufReq.size:%_", _buf_req.Length());
...
void ShortLink::__Run() {
    qm_xmessage2_define(message, TSF"taskid:%_, cgi:%_, @%_", task_.taskid, task_.cgi, this);
    qm_xinfo_function(TSF"%_, net:%_", message.String(), getNetInfo());
    ...
    conn_profile.nettype_for_report = marsMulti::comm::getNetTypeForStatistics();
    ...
    if(!is_keep_alive_) {
        qm_xinfo2(TSF"taskid:%_ sock %_ closed.", task_.taskid, fd_socket);
        socketOperator_->Close(fd_socket);
    } else {
        qm_xinfo2(TSF"keep alive, do not close socket:%_", fd_socket);
    }
}
...
SOCKET ShortLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    if (use_proxy_) {
        _conn_profile.proxy_info = marsMulti::app::GetProxyInfo(_conn_profile.host);
    }
    ...
    if (_conn_profile.ip_items.empty()){
        qm_xerror2(TSF"task socket connect fail %_ ipitems empty", message.String());
    ...
    if (use_proxy && _conn_profile.ip_items.front().source_type == kIPSourceDebug){
        qm_xwarn2(TSF"forbid proxy when debugip present.");
        use_proxy = false;
    }
    
    if (use_proxy && marsMulti::comm::kProxyHttp == _conn_profile.proxy_info.type && NetSource::GetShortLinkDebugIP().empty()) {
	...
	std::string proxy_ip;
    if (use_proxy && marsMulti::comm::kProxyNone != _conn_profile.proxy_info.type) {
        ...
        if (_conn_profile.proxy_info.ip.empty() && !_conn_profile.proxy_info.host.empty()) {
            if (!dns_util_.GetDNS().GetHostByName(_conn_profile.proxy_info.host, proxy_ips) || proxy_ips.empty()) {
                qm_xwarn2(TSF"dns %_ error", _conn_profile.proxy_info.host);
                return INVALID_SOCKET;
            }
    ...
    if (use_proxy && marsMulti::comm::kProxyHttp == _conn_profile.proxy_info.type) {
    ...
    if (use_proxy && (marsMulti::comm::kProxyHttpTunel == _conn_profile.proxy_info.type || marsMulti::comm::kProxySocks5 == _conn_profile.proxy_info.type)) {
		proxy_addr = &((new socket_address(proxy_ip.c_str(), _conn_profile.proxy_info.port))->v4tov6_address(local_stack));
        _conn_profile.ip_type = kIPSourceProxy;
    }

    qm_xinfo2_if(task_.priority >= 0, TSF"task socket dns sock %_ proxy:%_, host:%_, ip list:%_, is_keep_alive:%_", message.String(), kIPSourceProxy == _conn_profile.ip_type, _conn_profile.host, NetSource::DumpTable(_conn_profile.ip_items), is_keep_alive_);

    if (vecaddr.empty()) {
        qm_xerror2(TSF"task socket connect fail %_ vecaddr empty", message.String());
        __RunResponseError(kEctDns, kEctDnsMakeSocketPrepared, _conn_profile, false);
        delete proxy_addr;
        return INVALID_SOCKET;
    }
    ...
    _conn_profile.nettype_for_report = marsMulti::comm::getNetTypeForStatistics();
    __UpdateProfile(_conn_profile);
    ...
    if(_conn_profile.ip_type != kIPSourceProxy && is_keep_alive_) {
        for (size_t i = 0; i < _conn_profile.ip_items.size(); ++i) {
            ...
            if(fd != INVALID_SOCKET) {
                ...
                __UpdateProfile(_conn_profile);
                qm_xinfo2(TSF"reused socket:%_", fd);
                return fd;
            }
        }
    }
    ...
    if (outter_vec_addr_.empty()){
        ...
        qm_xdebug2(TSF"ip connect time: %_, %_", std::get<0>(ip_timeout), std::get<1>(ip_timeout));
    }else{
    ...
    if (INVALID_SOCKET == sock) {
        qm_xwarn2(TSF"task socket connect fail sock %_, net:%_", message.String(), getNetInfo());
    ...
    qm_xassert2(0 <= profile.index && (unsigned int)profile.index < _conn_profile.ip_items.size());
...
//    qm_xerror2_if(0 != setsockopt(sock, SOL_SOCKET, SO_LINGER, (const char*)&so_linger, sizeof(so_linger)), TSF"SO_LINGER %_(%_)", socket_errno, socket_strerror(socket_errno));
    return sock;
}
...
bool ShortLink::__ContainIPv6(const std::vector<socket_address>& _vecaddr) {
    if (!_vecaddr.empty()) {
        ...
        if (socket_inet_pton(AF_INET6, _vecaddr[0].ip(), &addr6)) { //first ip is ipv6
            qm_xinfo2_if(!task_.long_polling && task_.priority >= 0, TSF"ip %_ is v6", _vecaddr[0].ip());
            return true;
        }
    }
    return false;
...
void ShortLink::__RunReadWrite(SOCKET _socket, int& _err_type, int& _err_code, ConnectProfile& _conn_profile) {
	...
	if (_conn_profile.proxy_info.IsValid() && marsMulti::comm::kProxyHttp == _conn_profile.proxy_info.type
	...
	qm_xinfo2(TSF"task socket send sock:%_, %_ http len:%_, ", _socket, message.String(), out_buff.Length()) >> group_send;
    ...
    qm_xinfo2(TSF"sent %_", send_ret) >> group_send;
    
	if (send_ret < 0) {
		qm_xerror2(TSF"Send Request Error, ret:%0, errno:%1, nread:%_, nwrite:%_", send_ret, socketOperator_->ErrorDesc(_err_code), socket_nread(_socket), socket_nwrite(_socket)) >> group_send;
    ...
    if (socketOperator_->Breaker().IsBreak()) {
        qm_xwarn2(TSF"Send Request break, sent:%_ nread:%_, nwrite:%_", send_ret, socket_nread(_socket), socket_nwrite(_socket)) >> group_send;
        return;
    }

    task_.priority >= 0 ? (qm_xgroup2() << group_send) : (group_send.Clear());

	xgroup2_define(group_close);
	xgroup2_define(group_recv);
    
    qm_xinfo2(TSF"task socket close sock:%_, %_, ", _socket, message.String()) >> group_close;
	qm_xinfo2(TSF"task socket recv sock:%_,  %_, ", _socket, message.String()) >> group_recv;
	...
	while (true) {
        ...
        qm_xinfo2(TSF"socketOperator_ Recv %_/%_", recv_ret, _err_code);
		...
		if (recv_ret < 0) {
			qm_xerror2(TSF"read block socket return false, error:%0, nread:%_, nwrite:%_", socketOperator_->ErrorDesc(_err_code), socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
            is_keep_alive_ = false;
		...
		if (socketOperator_->Breaker().IsBreak()) {
			qm_xinfo2(TSF"user cancel, nread:%_, nwrite:%_", socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
        	_conn_profile.disconn_errtype = kEctCanceld;
			break;
		}

		if (recv_ret == 0 && SOCKET_ERRNO(ETIMEDOUT) == _err_code) {
			qm_xerror2(TSF"read timeout error:(%_,%_), nread:%_, nwrite:%_ ", _err_code, socketOperator_->ErrorDesc(_err_code), socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
		...
		if (recv_ret == 0 && socketOperator_->Protocol() != Task::kTransportProtocolQUIC) {
			qm_xerror2(TSF"remote disconnect error:(%_,%_), nread:%_, nwrite:%_", _err_code, socketOperator_->ErrorDesc(_err_code), socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
		...
		if (parse_status == http::Parser::kFirstLineError) {
			qm_xerror2(TSF"http head not receive yet,but socket closed, length:%0, nread:%_, nwrite:%_ ", recv_buf.Length(), socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
            ...
            if(is_keep_alive_) {    //parse server keep-alive config
                ...
                qm_xwarn2_if(!isKeepAlive, "request keep-alive, but server return close");
                if(isKeepAlive) {
                    ...
                    qm_xinfo2(TSF"svr keepalive %_ seconds for url %_:%_", timeout, _conn_profile.ip, _conn_profile.port);
                } else {
                    is_keep_alive_ = false;
                }
            }
	...
	qm_xdebug2(TSF"read with nonblock socket http response, length:%_, ", recv_buf.Length()) >> group_recv;

    task_.priority >= 0 ? (qm_xgroup2() << group_recv) : (group_recv.Clear());
#if defined(__ANDROID__) || defined(__APPLE__)
	struct tcp_info _info;
	if (getsocktcpinfo(_socket, &_info) == 0) {
		char tcp_info_str[1024] = {0};
		qm_xinfo2(TSF"task socket close getsocktcpinfo:%_", tcpinfo2str(&_info, tcp_info_str, sizeof(tcp_info_str))) >> group_close;
	}
#endif
	qm_xgroup2() << group_close;
}
...
void ShortLink::__OnResponse(ErrCmdType _errType, int _status, AutoBuffer& _body, AutoBuffer& _extension, ConnectProfile& _conn_profile, bool _report) {
    ...
    _conn_profile.channel_type = marsMulti::stn::Task::kChannelShort;
	__UpdateProfile(_conn_profile);

 //   qm_xassert2(!breaker_.IsBreak());

    if (kEctOK != _errType) {
//        qm_xassert2(func_network_report);
