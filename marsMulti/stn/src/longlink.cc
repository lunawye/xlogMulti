...
SOCKET LongLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    marsMulti::comm::ProxyInfo proxy_info = marsMulti::app::GetProxyInfo("");
    bool use_proxy = proxy_info.IsValid() && marsMulti::comm::kProxyNone != proxy_info.type && marsMulti::comm::kProxyHttp != proxy_info.type && netsource_.GetLongLinkDebugIP().empty();
