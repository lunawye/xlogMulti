...
def build_ios(tag=''):
    ...
    lipo_dst_lib = INSTALL_PATH + '/marsMulti'

    if not libtool_libs(lipo_src_libs, lipo_dst_lib):
        return False

    dst_framework_path = INSTALL_PATH + '/marsMulti.framework'
...
def build_ios_xlog(tag=''):
    ...
    libtool_src_libs = [INSTALL_PATH + '/libcomm.a',
                        INSTALL_PATH + '/libmarsMulti-boost.a',
...
def main():
    while True:
        if len(sys.argv) >= 2:
            ...
            num = raw_input('Enter menu:\n1. Clean && build marsMulti.\n2. Clean && build xlog.\n3. Gen iOS marsMulti Project.\n4. Exit\n')
            if num == '1':
                build_ios()
                break
            if num == '2':
