...
int PingQuery::RunPingQuery(int _querycount, int interval/*S*/, int timeout/*S*/, const char* dest, unsigned int packetSize, int* rtt) {
    qm_xinfo2(TSF"in runpingquery");
    qm_xassert2(_querycount >= 0, "ping count should be more than 0");
    qm_xassert2(interval >= 0, "interval should be more than 0");
    qm_xassert2(timeout >= 0, "timeout should be more than 0");
    ...
    if (NULL == dest || 0 == strlen(dest)) {
        ...
        if (-1 == ret) {
            qm_xerror2(TSF"get default gateway error.");
            return -1;
        }
        ...
        if (NULL == dest || 0 == strlen(dest)) {
            qm_xerror2(TSF"ping dest host is NULL.");
            return -1;
        }

        qm_xinfo2(TSF"get default gateway: %0", dest);
    }
    ...
    if (NULL != traffic_monitor_) {
        ...
        if (traffic_monitor_->sendLimitCheck(sendLen)) {  // 56 is default packet size
            qm_xwarn2(TSF"limitCheck!!!sendLen=%0", sendLen);
    ...
    if (strlen(dest) > 200) {
        qm_xerror2(TSF"domain name is too long.");
        return -1;
    }
    ...
    if (index < 0 || index >= 256) {
        qm_xerror2(TSF"sprintf return error.index=%_", index);
        return -1;
    }
    ...
    if (tempLen < 0 || tempLen >= 256 - index) {
        qm_xerror2(TSF"sprintf return error.tempLen=%_, index=%_", tempLen, index);
        return -1;
    }

    qm_xinfo2(TSF"popen cmd=%0", cmd);
    FILE* pp = popen(cmd, "r");

    if (!pp) {
        qm_xerror2(TSF"popen error:%0", strerror(errno));
        return -1;
    }
    ...
    if (pingresult_.empty()) {
        qm_xerror2(TSF"m_strPingResult is empty");
        return -1;
    }
    ...
    if (0 == pingStatusTemp.avgrtt && 0 == pingStatusTemp.maxrtt) {
        qm_xinfo2(TSF"remote host is not available, pingresult_:%_", pingresult_);
        return -1;
    }
    qm_xdebug2("ping rtt: %d, %d", (int)pingStatusTemp.avgrtt, (int)pingStatusTemp.maxrtt);
    *rtt = static_cast<int>(pingStatusTemp.avgrtt);

    qm_xinfo2(TSF"m_strPingResult = %0, %1", pingresult_, *rtt);
    return 0;
}
        ...
        if (vecPingRes.begin() == iter) {  // extract ip from the result string and assign to _ping_status.ip
            ...
            if (index1 > 0) {
                ...
                if (index2 > index1) {
                    ...
                    qm_xinfo2(TSF"ipTemp=%_, size = %_", ipTemp.c_str(), size);
                    qm_xassert2(size <= 16 && size > 0);
                    strncpy(_ping_status.ip, ipTemp.c_str(), (size < 16 ? size : 15));
                    qm_xdebug2(TSF"_ping_status.ip=%_", _ping_status.ip);
                }
            }
...
#if TARGET_OS_IPHONE
#include    "marsMulti/comm/objc/ip_icmp.h"
#else
#include    <netinet/ip_icmp.h>
#endif

#include "marsMulti/comm/qm_time_utils.h"  // comm/utils.h
...
static char* Sock_ntop_host(const struct sockaddr* sa, socklen_t salen) {
    ...
    if ((ptr = sock_ntop_host(sa, salen)) == NULL) {
        qm_xerror2(TSF"sock_ntop_host error,errno=%0", errno); /* inet_ntop() sets errno */
    }
...
static void Gettimeofday(struct timeval* tv, void* foo) {
    if (gettimeofday(tv, (struct timezone*)foo) == -1) {
        qm_xerror2(TSF"gettimeofday error");
    }
...
static int Sendto(int fd, const void* ptr, size_t nbytes, int flags, const struct sockaddr* sa, socklen_t salen) {
    ...
    if ((len = (int)sendto(fd, ptr, nbytes, flags, sa, salen)) != (ssize_t) nbytes) {
        qm_xerror2(TSF"sendto: uncomplete packet, len:%_, nbytes:%_, errno:%_(%_)", len, nbytes, socket_errno, strerror(socket_errno));
    }
...
static struct addrinfo* Host_serv(const char* host, const char* serv, int family, int socktype) {
    ...
    if ((n = getaddrinfo(host, serv, &hints, &res)) != 0) {
        qm_xerror2(TSF"host_serv error for %0, %1: %2",
...
void PingQuery::proc_v4(char* _ptr, ssize_t _len, struct msghdr* _msg, struct timeval* _tvrecv) {
    ...
    if ((icmplen = (int)_len - IP_HEADER_LEN) < ICMP_HEADER_LEN) {
        qm_xerror2(TSF"receive malformed icmp packet");
        return;             /* malformed packet */
    }

    // if (icmp->icmp_type == ICMP_ECHOREPLY)
    //  {
    qm_xdebug2(TSF"icmp->icmp_type=%0,is equal with ICMP_ECHOREPLY:%1", icmp->icmp_type, icmp->icmp_type == ICMP_ECHOREPLY);

    if (icmplen < ICMP_HEADER_LEN + sizeof(struct timeval)) {
        qm_xerror2(TSF"not enough data to compute RTT");
        return;         /* not enough data to use */
    }

    tvsend = (struct timeval*)(&_ptr[ICMP_MINLEN]);
    qm_xdebug2(TSF"before ntohl tvsend sec=%_, nsec=%_; tvrecv sec=%_, usec=%_", tvsend->tv_sec
            , tvsend->tv_usec, _tvrecv->tv_sec, _tvrecv->tv_usec);

    tvsend->tv_sec = ntohl(tvsend->tv_sec);
    tvsend->tv_usec = ntohl(tvsend->tv_usec);

    qm_xdebug2(TSF"tvsend sec=%_, nsec=%_; tvrecv sec=%_, usec=%_", tvsend->tv_sec
            , tvsend->tv_usec, _tvrecv->tv_sec, _tvrecv->tv_usec);
    ...
    if (rtt < 10000.0 && rtt > 0.0) {
        ...
        qm_xerror2(TSF"rtt = %0 is illegal.receive %1 bytes from %2", rtt, icmplen, Sock_ntop_host(&recvaddr_, sizeof(recvaddr_)));
    }
...
int PingQuery::__prepareSendAddr(const char* _dest) {
    ...
    qm_xinfo2(TSF"PING %0 (%1): %2 data bytes\n", (ai->ai_canonname ? ai->ai_canonname : h), h, DATALEN);

    if (ai->ai_family != AF_INET) {
        qm_xinfo2(TSF"unknown address family %0\n", ai->ai_family);
        freeaddrinfo(ai);
        return -1;
    }

    memcpy(&sendaddr_, ai->ai_addr, sizeof(struct sockaddr));
    qm_xdebug2(TSF"m_sendAddr=%0", socket_address(&sendaddr_).ip());
...
int PingQuery::__initialize(const char* _dest) {
    ...
    if (ret != 0) {
        qm_xerror2(TSF"__initialize():set nonblock socket error:%0", socket_strerror(socket_errno));
        return -1;
    }
...
int PingQuery::__recv() {
    ...
    if (NULL != traffic_monitor_) {
        if (traffic_monitor_->recvLimitCheck(n)) {
            qm_xwarn2(TSF"limitCheck,recv Size=%0", n);
            return TRAFFIC_LIMIT_RET_CODE;
        }
    }

    qm_xinfo2(TSF"after recvmsg() n =%0\n", (int)n);
    ...
    qm_xdebug2(TSF"gettimeofday sec=%0,usec=%1", tval.tv_sec, tval.tv_usec);
...
int PingQuery::__send() {
    ...
    if (NULL != traffic_monitor_) {
        if (traffic_monitor_->sendLimitCheck(len)) {
            qm_xwarn2(TSF"limitCheck!!len=%0", len);
            return TRAFFIC_LIMIT_RET_CODE;
        }
    }
...
void PingQuery::__preparePacket(char* _sendbuffer, int& _len) {
    ...
    qm_xdebug2(TSF"gettimeofday now sec=%0, nsec=%1", now.tv_sec, now.tv_usec);
...
int PingQuery::__runReadWrite(int& _errcode) {
    ...
    while (readcount_ > 0 && sel_timeout_cnt < 10) {
        ...
        if (retsel < 0) {
            qm_xerror2(TSF"retSel<0");
            _errcode = sel.Errno();
            return -1;
        }
        
        if (sel.IsBreak()){
            qm_xinfo2(TSF"user breaked");
            _errcode = EINTR;
            return -1;
        }

        if (sel.IsException()) {
            qm_xerror2(TSF"socketselect exception");
...
int PingQuery::RunPingQuery(int _querycount, int _interval/*S*/, int _timeout/*S*/, const char* _dest, unsigned int _packet_size) {
    qm_xassert2(_querycount >= 0);
    qm_xdebug2(TSF"dest=%0", _dest);
    ...
    if (NULL == _dest || 0 == strlen(_dest)) {
        ...
        qm_xinfo2(TSF"get default gateway: %0", _dest);
    }
...
int PingQuery::GetPingStatus(struct PingStatus& _ping_status) {
    ...
    qm_xdebug2(TSF"pingIP=%0", pingIP);

    if (pingIP != NULL) {
        strncpy(_ping_status.ip, pingIP, 16);
    } else {
        qm_xerror2(TSF"pingIP==NULL");
    }

    qm_xinfo2(TSF"getPingStatus():size = %0; m_readCount=%1,m_sendTimes=%2", size, readcount_, sendtimes_);
...
int doPing(const std::string& _destaddr, std::string& _real_pingip, std::string& _resultstr, unsigned int _packet_size, int _pingcount, int _interval/*s*/, int _timeout/*s*/) {
    ...
    qm_xdebug2(TSF"realPingIP=%0,resultStr=%1,destAddr=%2", _real_pingip.c_str(), _resultstr.c_str(), _destaddr.c_str());
...
int PingQuery::RunPingQuery(int _querycount, int interval/*S*/, int timeout/*S*/, const char* dest, unsigned int packetSize) {
    qm_xerror2(TSF"ping query is not support on win32 now!");
    return 0;
}
...
int PingQuery::RunPingQuery(int _querycount, int interval/*S*/, int timeout/*S*/, const char* dest, unsigned int packetSize) {
    qm_xerror2(TSF"ping query is not support  now!");
    return 0;
}
int PingQuery::GetPingStatus(struct PingStatus& pingStatus) {
    qm_xerror2(TSF"ping query is not support now!");
