...
void TcpClientFSM::Close(bool _notify) {
    ...
    if (remote_close_ || 0 != error_) {
        qm_xinfo2(TSF"sock:%_, (%_:%_), close local socket close, notify:%_", sock_, addr_.ip(), addr_.port(), _notify);
        socket_close(sock_);
        sock_ = INVALID_SOCKET;
        return;
    }
...
void TcpClientFSM::PreSelect(SocketSelect& _sel, QM_XLogger& _log) {
    ...
    switch(status_) {
        ...
        default:
            qm_xassert2(false, "preselect status error");
    }
}

void TcpClientFSM::AfterSelect(SocketSelect& _sel, QM_XLogger& _log) {
...
void TcpClientFSM::PreConnectSelect(SocketSelect& _sel, QM_XLogger& _log) {
    qm_xassert2(EStart == status_, "%d", status_);
    _OnCreate();

    qm_xinfo2(TSF"addr:(%_:%_), ", addr_.ip(), addr_.port()) >> _log;
    ...
    if (sock_ == INVALID_SOCKET) {
        ...
        status_ = EEnd;
        _OnClose(last_status_, error_, false);
        qm_xerror2(TSF"close socket err:(%_, %_)", error_, socket_strerror(error_)) >> _log;
        return;
    }
...
#ifdef _WIN32
    if (0 != socket_ipv6only(sock_, 0)){ qm_xwarn2(TSF"set ipv6only failed. error %_",strerror(socket_errno)); }
#endif
    
    if (0 != socket_set_nobio(sock_)) {
        error_ = socket_errno;
        qm_xerror2(TSF"close socket_set_nobio:(%_, %_)", error_, socket_strerror(error_)) >> _log;
    } else {
        qm_xinfo2(TSF"sock:%_, ", sock_) >> _log;
    }
    ...
    if (0 != ret && !IS_NOBLOCK_CONNECT_ERRNO(socket_errno)) {
        ...
        error_ = socket_errno;
        qm_xwarn2(TSF"close connect err:(%_, %_), localip:%_", error_, socket_strerror(error_), socket_address::getsockname(sock_).ip()) >> _log;
    } else {
        qm_xinfo2("connect") >> _log;
        _sel.Write_FD_SET(sock_);
        _sel.Exception_FD_SET(sock_);
    }
...
void TcpClientFSM::AfterConnectSelect(const SocketSelect& _sel, QM_XLogger& _log) {
    qm_xassert2(EConnecting == status_, "%d", status_);
    ...
    if (_sel.Exception_FD_ISSET(sock_)) {
        ...
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, addr_.ip(), addr_.port()) >> _log;
        qm_xwarn2(TSF"close connect exception: sock:%_, err(%_, %_)", sock_, error_, socket_strerror(error_)) >> _log;
    ...
    qm_xinfo2(TSF"socket error:%_, ", error_) >> _log;
    
    if (0 != error_) {
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, addr_.ip(), addr_.port()) >> _log;
        qm_xwarn2(TSF"close connect error:(%_, %_), ", error_, socket_strerror(error_)) >> _log;
        end_connecttime_ = gettickcount();
        last_status_ = status_;
        status_ = EEnd;
        return;
    ...
    if (0 == error_ && _sel.Write_FD_ISSET(sock_)){
        ...
        status_ = EReadWrite;
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, addr_.ip(), addr_.port()) >> _log;
        qm_xinfo2(TSF"connected Rtt:%_, ", Rtt()) >> _log;
        _OnConnected(Rtt());
        return;
    }
    ...
    if (0 >= timeout) {
        end_connecttime_ = gettickcount();
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, addr_.ip(), addr_.port()) >> _log;
        qm_xwarn2(TSF"close connect timeout:(%_, %_), (%_, %_)", ConnectAbsTimeout(), -timeout, SOCKET_ERRNO(ETIMEDOUT), socket_strerror(SOCKET_ERRNO(ETIMEDOUT))) >> _log;
...
void TcpClientFSM::PreReadWriteSelect(SocketSelect& _sel, QM_XLogger& _log) {
    qm_xassert2(EReadWrite == status_, "%d", status_);
...
void TcpClientFSM::AfterReadWriteSelect(const SocketSelect& _sel, QM_XLogger& _log) {
    qm_xassert2(EReadWrite == status_, "%d", status_);
    ...
    if (_sel.Exception_FD_ISSET(sock_)) {
        ...
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
        qm_xwarn2(TSF"close exception:(%_, %_), ", error_, socket_strerror(error_)) >> _log;
        last_status_ = status_;
        status_ = EEnd;
        return;
    }
    ...
    if (_sel.Write_FD_ISSET(sock_)) {
        ...
        if (0 < ret) {
            ...
            status_ = EEnd;
            qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
            qm_xwarn2(TSF"close send err:(%_, %_, %_), localip:%_", ret, error_, socket_strerror(error_),socket_address::getsockname(sock_).ip()) >> _log;
            return;
        }
    }
    ...
    if (_sel.Read_FD_ISSET(sock_)) {
        ...
        if (0 < ret) {
            ...
            remote_close_ = true;
            qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
            qm_xwarn2(TSF"close recv %_:(%_, %_, %_)", "remote socket close", ret, 0, socket_strerror(0)) >> _log;
            return;
            ...
            status_ = EEnd;
            qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
            qm_xwarn2(TSF"close recv %_:(%_, %_, %_), localip:%_", "err", ret, error_, socket_strerror(error_), socket_address::getsockname(sock_).ip()) >> _log;
            return;
        }
    }
    ...
    if (!_sel.Write_FD_ISSET(sock_) && !_sel.Read_FD_ISSET(sock_) && 0 >= timeout) {
        qm_xinfo2(TSF"sock:%_, (%_:%_), ", sock_, IP(), Port()) >> _log;
        qm_xwarn2(TSF"close readwrite timeout:(%_, %_), (%_, %_)", ReadWriteAbsTimeout(), -timeout, SOCKET_ERRNO(ETIMEDOUT), socket_strerror(SOCKET_ERRNO(ETIMEDOUT))) >> _log;
