...
QM_XLogger::QM_XLogger(TLogLevel _level, const char* _tag, const char* _file, const char* _func, int _line, bool _trace, bool (*_hook)(XLoggerInfo& _info, std::string& _log))
...
void QM_XLogger::operator>> (QM_XLogger& _xlogger) {
...
QM_XLogger& QM_XLogger::Assert(const char* _exp) {
	m_isassert = true;
	m_exp = _exp;
	return *this;
}

QM_XLogger& QM_XLogger::operator<< (const string_cast& _value) {
	if (NULL != _value.str()) {
...
QM_XLogger& QM_XLogger::VPrintf(const char* _format, va_list _list) {
	if (_format == NULL)
	{
		m_info.level = kLevelFatal;
...
QM_XLogger& QM_XLogger::operator()(const char* _format, ...) {
	if (_format == NULL)
	{
		m_info.level = kLevelFatal;
...
void QM_XLogger::DoTypeSafeFormat(const char* _format, const string_cast** _args) {
