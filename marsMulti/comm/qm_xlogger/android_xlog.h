...
#include "qm_xlogger/qm_xloggerbase.h"
...
static void __ComLogV(int level, const char *tag, const char* file, const char* func, int line, const char* fmt, va_list args) {
	...
	//important: do not use like  this:qm_xlogger2((TLogLevel)level, tag, file, func, line, fmt, args);
	//qm_xlogger2((TLogLevel)level, tag, file, func, line).VPrintf(fmt, args);
}
