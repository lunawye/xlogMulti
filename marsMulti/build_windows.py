...
def build_windows(incremental, tag='', config='Release'):
    ...
    merge_win_static_libs(src_libs, WIN_RESULT_DIR + 'marsMulti.lib')
...
def build_windows_xlog(incremental, tag='', config='Release'):
    ...
    needed_libs = [os.path.normpath(WIN_LIBS_INSTALL_PATH + 'comm.lib'), os.path.normpath(WIN_LIBS_INSTALL_PATH + 'marsMulti-boost.lib'), os.path.normpath(WIN_LIBS_INSTALL_PATH + 'xlog.lib')]
...
def main():
    ...
    while True:
        if len(sys.argv) >= 3:
            ...
            '1. Clean && build marsMulti Release.\n2. Build marsMulti Release incrementally.\n3. Clean && build marsMulti Debug.\n'\
            '4. Build marsMulti Debug incrementally.\n5. Clean && build xlog Release.\n6. Clean && build xlog Debug.\n7. Gen marsMulti project file.\n8. Exit\n')
            if num == '1':
