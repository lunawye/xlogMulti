...
void CPingServerDlg::OnResponse(HelloCGITask* task, const com::tencent::marsMulti::sample::proto::HelloResponse& response)
{
	std::wstring info = std::wstring(L"Recv Hello Response: ") + (wchar_t*)(_bstr_t)response.errmsg().c_str() + L"\r\n";
	m_editInfo.AppendText(info.c_str());
}

void CPingServerDlg::OnResponse(GetConvListCGITask* task, const com::tencent::marsMulti::sample::proto::ConversationListResponse& response)
{
	...
	{
		const com::tencent::marsMulti::sample::proto::Conversation& item = response.list(i);
