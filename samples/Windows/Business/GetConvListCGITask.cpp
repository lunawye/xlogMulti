...
#include <marsMulti/comm/windows/projdef.h>
...
{
	string data;
	com::tencent::marsMulti::sample::proto::ConversationListRequest request;
	request.set_access_token(access_token_);
	request.set_type(com::tencent::marsMulti::sample::proto::ConversationListRequest_FilterType_DEFAULT);
...
{
	com::tencent::marsMulti::sample::proto::ConversationListResponse response;
	response.ParseFromArray(_inbuffer.Ptr(), _inbuffer.Length());
	if (callback_.lock())callback_.lock()->OnResponse(this, response);
	return marsMulti::stn::kTaskFailHandleNoError;
}
