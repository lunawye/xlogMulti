...
-(void) doAFNetworking {
    ...
    [manager POST:@"http://118.89.24.72:8080/marsMulti/hello2" parameters:[helloRequest data] progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
-(void) StartMarsTest {
    scene = SceneMars;
    NSLog(@"benchmark marsMulti start");
    
    start_time = [[NSDate date] timeIntervalSince1970] * 1000;
    task_time = [[NSDate date] timeIntervalSince1970] * 1000;
    task_cnt = task_suc = suc_time = 0;
    CGITask *helloCGI = [[CGITask alloc] initAll:ChannelType_ShortConn AndCmdId:kSayHello2 AndCGIUri:@"/marsMulti/hello2" AndHost:@"118.89.24.72"];
...
- (int)onTaskEnd:(uint32_t)tid errType:(uint32_t)errtype errCode:(uint32_t)errcode {
    ...
    if(scene == SceneSensitivity) {
        NSLog(@"benchmark marsMulti errtype:%d, errcode:%d, cost:%f", errtype, errcode, ([[NSDate date] timeIntervalSince1970] * 1000 - task_time));
        return 0;
    }
    ...
    if(errtype == 0 && errcode == 0) {
        ...
        //NSLog(@"benchmark marsMulti suc cost:%llu, cnt:%llu, suc cnt:%llu, total:%llu", (curr - task_time), task_cnt, task_suc, (curr - start_time));
    } else {
        NSLog(@"benchmark marsMulti fail cnt:%llu, cost:%llu, errtype:%d, errcode:%d", task_cnt, (curr-task_time), errtype, errcode);
    }
    
    if((task_suc%50) == 0) {
        NSLog(@"benchmark marsMulti total:%llu, avg:%llu, suc_rate:%f, suc:%llu, cnt:%llu", (curr - start_time), (suc_time)/task_suc, task_suc/(double)task_cnt, task_suc, task_cnt);
    }
    if(task_suc >= 500) {
        NSLog(@"benchmark marsMulti final total:%llu, avg:%llu, suc_rate:%f, suc:%llu", (curr - start_time), (suc_time)/task_suc, task_suc/(double)task_cnt, task_suc);
        return 0;
    }
...
-(NSData*)requestSendData {
    ...
    //Benchmark scene: 64KB,128KB request test
    //HelloRequest* helloRequest = [[[[[HelloRequest builder] setUser:@"marsMulti"] setText:@"Hello marsMulti!"] setDumpContent:[self makeDumpData:128*1024]] build];
...
-(void) StartSensitivityTest {
    ...
    [manager.requestSerializer setValue:@"application/octet-stream" forHTTPHeaderField:@"Content-Type"];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/octet-stream", nil];
    [manager POST:@"http://118.89.24.72:8080/marsMulti/hello2" parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
