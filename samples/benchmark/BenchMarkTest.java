...
//package com.tencent.marsMulti.sample;
...
import com.tencent.marsMulti.sample.proto.Main;
import com.tencent.marsMulti.sample.wrapper.remote.MarsServiceProxy;
import com.tencent.marsMulti.sample.wrapper.remote.NanoMarsTaskWrapper;
import com.tencent.marsMulti.xlog.Log;
    ...
    private class HelloTaskWrapper extends NanoMarsTaskWrapper<Main.HelloRequest, Main.HelloResponse> {
        ...
        public void onTaskEnd(final int errType, final int errCode) {
            if(scene == BenchMarkScene.SceneSensitivity) {
                Log.i(TAG, "marsMulti cost:" + (System.currentTimeMillis() - task_time) + "errtype:" + errType + ", errcode:" + errCode + ", msg:" + response.errmsg);
                return;
            }
            ...
            if(errType == 0 && errCode == 0 && response.retcode == 0) {
                ...
                //Log.i(TAG, "marsMulti suc cost:" + (curr - task_time) + ", count:" + task_cnt + ", suc count:" + task_suc + ", msg:" + response.errmsg + ", total:" + (curr - start_time));
            } else {
                Log.e(TAG, "marsMulti fail cost:" + (curr - task_time) + ", count:" + task_cnt + ", total:" + (curr - start_time));
            }

            if((task_suc%50) == 0) {
                Log.e(TAG, "marsMulti total:" + (curr - start_time) + ", avg:" + (suc_time)/(double)task_suc + ",rate:" + task_suc/(double)task_cnt + ", count:" + task_cnt + ", task suc:" + task_suc);
            }

            if(task_suc < 500) {
                task_time = System.currentTimeMillis();
                HelloTaskWrapper task = new HelloTaskWrapper();
                MarsServiceProxy.send(task.setHttpRequest("118.89.24.72", "/marsMulti/hello2").setCmdID(8));
            } else {
                String msg = "final marsMulti total:" + (curr - start_time) + ", avg:" + (suc_time)/(double)task_suc + ",rate:" + task_suc/(double)task_cnt + ", count:" + task_cnt + ", suc:" + task_suc;
                Log.e(TAG, msg);
            }
        }
    ...
    void doOkHttpRequest() {
        ...
        try {
            ...
            Request request = new Request.Builder()
                    .url("http://118.89.24.72:8080/marsMulti/hello2")
